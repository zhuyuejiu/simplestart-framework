...
public class PackageScanUtils {
	...
	private static Set<String> getClassNameFromJar(Enumeration<JarEntry> jarEntries, String packageName,
		...
		while (jarEntries.hasMoreElements()) {
			...
			if (!jarEntry.isDirectory()) {
				/*
				 * 这里是为了方便，先把"/" 转成 "." 再判断 ".class" 的做法可能会有bug (FIXME: 先把"/" 转成 "." 再判断 ".class" 的做法可能会有bug)
				 */
	 ...
	 * </pre>
	 * 
	 * @param packageName 该参数为一组包路径
	 * @param isRecursion 该参数的作用指定是否读取子包的类
	 * @return 返回该组包路径下全部的类的全限制名
	 */
	public static Set<String> getClassNames(String[] packageNames, boolean isRecursion) {
		...
		for (int i = 0; i < length; i++) {
			Set<String> classNameSet = PackageScanUtils.getClassName(packageNames[i], isRecursion);
			// 每次都将包放在总的Set集合中
